...
import { Box, Typography, TextField, Grid, Select, MenuItem, styled,createTheme, Divider, Button, IconButton } from '@mui/material';
...
const DialogOneTimeLink = (props) => {
    ...
    const handleCompletedBurnLink = async(password: string = "") => {
        ...
        return await burnOneTimeLink({
            ...
            onCompleted:(response)=>{
                if(response && response?.getOneTimeLinkDetails?.message === "Success" && response?.getOneTimeLinkDetails?.code === "200")
                {
                ...
                errorMessage(response?.getOneTimeLinkDetails?.message || "Something went wrong!", 3000);

                return false;
            },
            onError: (error) => {
                errorMessage(error?.message || "Burning the secret Url is incompleted", 3000);
                return false;
            }
        })
    }
    ...
    <React.Fragment>
        ...
        >
            ...
            <Box style={{padding: 15, margin:'15px', borderRadius: '10%', border:2}}>
            {
                ...
                (
                    <Box sx={{ paddingY: 3 }}>
                        <Typography variant="h6" sx={{width: '100%', mb: 4, textAlign:'center'}}>Your Onetime Secret URL</Typography>
                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 3, my: 2 }}>
                            <Typography sx={{padding:'0.1rem .7rem',fontWeight: 600, backgroundColor: 'rgba(174, 247, 40, 0.3)'}}>{generatedLink}</Typography>
                            <Button variant="contained" onClick={()=>handleCopy(generatedLink)} color="primary" sx={{width:'auto', alignSelf:'end'}}>Copy</Button>
                        </Box>
                        <Divider sx={{ marginY: 2 }} />
                        <Box sx={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:5, border: '1px solid #d6d6d6', borderRadius: '7px', padding: 7}}>
                            <div
                                ref={qrCodeRef}
                                style={{
                                display: "flex",
                            ...
                            <Box sx={{padding: 2}}>
                                <Typography sx={{mb: 4}}>
                                    This link 
                                    <span style={{color: '#2e7d32', margin:'0 4px'}}>{generatedLink}</span> 
                                    will be expired on 
                                    <span style={{color: '#2e7d32', margin:'0 5px'}}>{expiredAt}</span>
                                </Typography>
                                <Box sx={{display: 'flex', gap: 5, position: "relative"}}>
                                    <ButtonContainer 
                                        ...
                                        color="success"
                                        onClick={(e) =>
                                            handleDownloadQRCode(e, qrCodeRef, {title: 'Secret Url', description: 'This link will be automatically expired'})
                                        }
                                    >
                                        Download
                                    </ButtonContainer>
                                    <ButtonContainer 
                                        ...
                                        onClick={async (e) => {
                                            if (isShared) {
                                              ...
                                              await handleShareQR(e, qrCodeRef, {title: 'Secret Url', description: 'This link will be automatically expired'});
                        ...
                        <Box sx={{mt: 5,px: 7,py: 4,border: '1px solid #d6d6d6', borderRadius: '7px'}}>
                            <ButtonContainer
                                variant="contained"
                            ...
                            * Burning this secret will delete it before it has been read (click to cofnirm).
                        </Box>
                    </Box>
                )
            }
